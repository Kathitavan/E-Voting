...
function PartyLogo({ src, alt, emoji, className }) {
  ...
  if (failed) return <div className={`${className} av-logo-fallback`}>{emoji}</div>;
    ...
    if (p === "selecting") {
      ...
      if (res.direction !== "neutral") {
        ...
        if (t.dir !== res.direction) {
          ...
          if (res.direction === "up")   setSelectedIndex(i => Math.max(0, i - 1));
          else                          setSelectedIndex(i => Math.min(candidates.length - 1, i + 1));
        }
      } else {
        tiltRef.current = { dir: null, ts: 0 };
      ...
      if (res.direction !== "neutral") {
        ...
        if (t.dir !== res.direction) {
          ...
          tiltRef.current = { dir: null, ts: 0 };
          if      (res.direction === "up")   castVote(pendingRef.current);
          else if (res.direction === "down")  cancelConfirm();
        }
      } else {
        tiltRef.current = { dir: null, ts: 0 };
      }
          ...
          <div className="av-dialog">
            ...
            <div className="av-dlg-party">
              <div className="av-dlg-logo">
                <PartyLogo src={pendingCandidate.logo} alt={pendingCandidate.short} emoji={pendingCandidate.emoji} className="av-dlg-logo-img"/>
              </div>
          ...
          <div className="av-cam-col">
            ...
            <div className="av-current">
              ...
              <div className="av-current-card">
                <PartyLogo src={selCandidate.logo} alt={selCandidate.short} emoji={selCandidate.emoji} className="av-current-logo"/>
                <div>
            ...
            <div className="av-legend">
              <div className="av-legend-row"><span className="av-legend-arrow av-legend-arrow--up">↑</span><span>Tilt head <strong>UP</strong> — previous party</span></div>
              <div className="av-legend-row"><span className="av-legend-arrow av-legend-arrow--down">↓</span><span>Tilt head <strong>DOWN</strong> — next party</span></div>
              <div className="av-legend-row"><span className="av-legend-arrow av-legend-arrow--blink">👁</span><span><strong>Hold blink 1s</strong> — open confirm</span></div>
              <div className="av-legend-row"><span className="av-legend-arrow av-legend-arrow--up">↑</span><span>In popup: tilt <strong>UP</strong> to confirm</span></div>
              <div className="av-legend-row"><span className="av-legend-arrow av-legend-arrow--down">↓</span><span>In popup: tilt <strong>DOWN</strong> to cancel</span></div>
            </div>
          </div>
              ...
              {candidates.map((c, i) => (
                <div key={c.short} data-idx={i}
                  ...
                  <div className="av-card-logo">
                    <PartyLogo src={c.logo} alt={c.short} emoji={c.emoji} className="av-card-logo-img"/>
                  </div>
