...
#  Whatever the QR contains (URL, raw bytes, plain text) → stable
#  64-char SHA-256 hex used as the voter DB key.
#  Fixes: "http://robslink.com/..." stored literally as a key.
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
#  PASSIVE ANTI-SPOOFING  —  Liveness detection (no extra model needed)
#
#  Runs four independent texture/signal checks on the face ROI.
#  All four must pass before face recognition is attempted.
#
#  Detects:
...
def check_liveness(img_bgr: np.ndarray) -> tuple:
    ...
    Check 1 – Laplacian texture variance
      Real faces have micro-texture (pores, hair, skin detail).
      Photos and screens flatten this; Laplacian variance drops sharply.

    Check 2 – HSV saturation naturalness
      Screens often show high uniform saturation.
      Printed B&W photos show near-zero saturation.
      Real skin has moderate, naturally spread saturation values.

    Check 3 – Specular highlight ratio
      Lit screens produce a large near-white rectangle.
      Real faces have small, scattered highlights only.

    Check 4 – Gradient edge sharpness
      Printed edges and screen pixel grids produce unnaturally
      high mean gradient with very low standard deviation.
      Real facial features have smooth gradient transitions.
    """
    ...
    # ── Check 2: Colour naturalness (HSV saturation spread) ─────────
    ...
    # ── Check 3: Specular highlights (screen glare) ──────────────────
    ...
    # ── Check 4: Gradient edge naturalness ───────────────────────────
    ...
    print(f"[liveness] PASS  lap={lap_var:.1f}  s_std={s_std:.1f}  "
          f"bright={bright_ratio:.3f}  g_mean={g_mean:.1f}")
    return True, "live"
...
def register():
    ...
    voter_id = make_voter_id(qr_raw)      # ← SHA-256 of raw QR payload
    ...
    # Liveness check — prevent registering with a photo
...
# ══════════════════════════════════════════════════════════════════════
#  VERIFY QR  — key derived via make_voter_id()
# ══════════════════════════════════════════════════════════════════════
...
def verify_qr():
    ...
    return jsonify({
        "status":     "success",
        "qr_string":  voter_id,           # downstream routes use voter_id, not raw QR
        "voter_info": voter,
    })
...
def verify_face():
    data       = request.get_json(silent=True)
    voter_id   = (data or {}).get("qr_string", "").strip()
    ...
    # ── Liveness gate — must pass before any recognition ────────────
...
def admin_results():
    ...
    voted    = load_json("voted_status.json")
    votes: dict        = {}
    gender_stats: dict = {"male": 0, "female": 0, "other": 0}
